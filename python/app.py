...
PUBLIC_ENDPOINTS = {'login', 'api_login', 'static', 'favicon'}
...
def estimate_pool_temperature(state: dict | None = None, services: dict | None = None,
    ...
    cooling = min(8.0, hourly_loss * hours)
    return {
        'temp_c': round(max(4.0, baseline - cooling), 1),
        'source': source,
        'baseline_c': round(baseline, 1),
...
DEFAULT_LOAD_CURVE_KWH = [
    0.30, 0.30, 0.30, 0.30, 0.30, 0.30,   # 00-05  Nacht/Grundlast
    0.40, 0.55, 0.50, 0.40, 0.35, 0.40,   # 06-11  Morgen
    0.45, 0.40, 0.35, 0.35, 0.40, 0.65,   # 12-17  Mittag/Nachmittag
    0.95, 1.20, 0.90, 0.60, 0.40, 0.30,   # 18-23  Abend (Herd, Waesche, TV)
]
...
def merged_load_curve(profile: dict | None) -> list:
    ...
    for h in range(24):
        ...
        if avg_w is not None and (cnt > 0 or entry.get('source') == 'learned_blend'):
            kwh = avg_w / 1000.0
            # Plausibilitaetscheck (zwischen 0.05 und 5 kWh / Stunde)
            if 0.05 <= kwh <= 5.0:
...
def build_battery_plan(services: dict) -> dict:
    ...
    plan = {
        ...
        'overshoot': sim_soc > dyn_target + 2.0,
...
def apply_battery_control(plan: dict):
    """Setzt InWRte (Lade-Limit) am Akku gemaess Plan.

    overshoot   -> set_charge_limit_pct(0)   - Akku darf nicht weiter laden
    undershoot  -> release()                  - alles wieder freigeben
    sonst       -> release()
    """
    ...
    try:
        ...
        if plan.get('overshoot'):
            if _last_battery_action != {'action': 'limited', 'pct': 0}:
                _battery_modbus.set_charge_limit_pct(0, revert_seconds=0)
                _last_battery_action = {'action': 'limited', 'pct': 0}
                logger.info(f'Battery LIMIT 0% (proj_soc {plan["projected_soc_1830"]}%>{plan["target_soc_1830"]}%)')
        else:
            if _last_battery_action['action'] != 'released':
                _battery_modbus.release()
                _last_battery_action = {'action': 'released', 'pct': 100}
                logger.info('Battery release (kein overshoot)')
    except Exception as e:
...
def get_services():
    ...
    global tibber_service, forecast_service, weather_service, evcc_service, optimizer_service, learning_service
    ...
    if optimizer_service is None:
        optimizer_service = Optimizer(
            ...
            max_charge_power=float(os.getenv('WATTPLOT_MAX_KW', 22)) * 1000  # kW to W
        )
...
def api_set_threshold():
    ...
    threshold = data.get('threshold', 28.0)
    
    try:
        threshold = float(threshold)
        if 5 <= threshold <= 100:
            price_threshold = threshold
            logger.info(f"Price threshold updated to {price_threshold:.1f} ct/kWh")
            return jsonify({'success': True, 'threshold': price_threshold})
    except:
        pass
...
def api_chart():
    ...
    try:
        ...
        if rng == 'today':
            ...
            target_idx = min(bucket_count - 1, int((18 * 60 + 30) / bucket_min))
            end_soc = max(5.0, target_soc - 12.0)
            expected_profile = {}
            try:
            ...
            for i, label in enumerate(full_labels):
                if last_idx >= 0 and i >= last_idx and last_soc is not None:
                    if i <= target_idx:
                        span = max(1, target_idx - last_idx)
                        t = (i - last_idx) / span
                        expected_soc.append(round(last_soc + (target_soc - last_soc) * t, 1))
                    else:
                        span = max(1, bucket_count - 1 - target_idx)
                        t = (i - target_idx) / span
                        expected_soc.append(round(target_soc + (end_soc - target_soc) * t, 1))
                else:
                ...
                if label in series['load']:
                    ...
                    entry = expected_profile.get(hour) or expected_profile.get(str(hour)) or {}
                    expected_load.append(round(abs(float(entry.get('avg_consumption_w') or 400)), 1))
                elif 18 <= hour <= 21:
                    expected_load.append(900)
                elif 6 <= hour <= 8:
                    expected_load.append(500)
                elif 9 <= hour <= 17:
                    expected_load.append(380)
                else:
                    expected_load.append(280)
